...
def save_jobs_to_google_sheet(jobs_data: list[dict]):
    ...
    current_timestamp = datetime.datetime.now().strftime("%m/%d/%y")
    values_to_append = []
    for job in jobs_data:
        values_to_append.append([
            ...
            job.get('url', 'N/A'),
            job.get('status', 'Ready for Review'),  # Default status
        ])
    ...
    try:
        print(f"Google Sheets: Appending {len(values_to_append)} rows to ID: {spreadsheet_id}, Sheet: {sheet_name_to_use}.")
...
if __name__ == '__main__':
    print("Testing Google Sheets utility for intern-agent...")
    ...
    if not os.getenv('SPREADSHEET_ID') or not os.getenv('GOOGLE_APPLICATION_CREDENTIALS'):
        ...
        sample_jobs = [
            {'company': 'Test Co', 'role': 'Intern', 'location': 'Remote', 'date_posted': 'Jun 01', 'url': 'http://example.com/a'},
            {'company': 'Test Corp B', 'role': 'Intern', 'location': 'Local', 'date_posted': 'May 30', 'url': 'http://example.com/b'}
        ]
